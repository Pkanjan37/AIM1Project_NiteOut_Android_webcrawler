...
def setupDB(db,mode):
    ...
    if mode == 1:
        ...
        inSQL = "INSERT INTO \"OPENING_HOUR\" (\"DAY\",\"RESTAURANT_ID\",\"OPENING_TIME\",\"CLOSING_TIME\")VALUES('"+db.day+"','"+str(db.restaurantID)+"','"+db.openingtimes+"','"+db.closetimes+"');"
        #print ("old SQL : " + inSQL)
        #print ("new SQL : "+ inSQL)
    ...
    try:
        with open('/home/p/django/aim1/mysite/polls/insert_script.sql', 'a') as the_file:
            the_file.write(inSQL+'\n')
        ...
        cur = conn.cursor()
        #cur.execute(open("/home/p/Documents/aim1/Untitled Folder/information-research/NiteOut.Data.ChangeScripts/002_NightOut_DB.sql", "r").read())
        cur.execute(inSQL)
        #print(inSQL)
        conn.commit()
...
def apiCall(request):
    ...
    while (checkerYelp):
        ...
        for i in xrange(0, len(x.businesses)):
            ...
            print("================+++")
            print(x.businesses[i].id)
            print("================+++")
            respR = requests.get('https://api.yelp.com/v3/businesses/' + str(x.businesses[i].id) + '/reviews',
                                 headers={
                                     'Authorization': 'Bearer sfNwLics9lPqpWB7916BaiwHJTYVw7a2jlWbcxbwJuvdmvCB4flz1Hv0tPL3tZQpgl88DoGytEUEJCRrtChhSAU7rZ5KBOlidyyVY24TzREO_yc0IiX3iLWpnAw5WnYx'}, )
